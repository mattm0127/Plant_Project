...
class SensorController:
    ...
    def run_sensor_server(self):
        try:
            ...
            while True:
                if wifi.radio.connected == False:
                    ...
                    try:
                        ...
                        microcontroller_timeout += 1
                        if microcontroller_timeout > 5:
                            raise Exception("WiFi Reconnect Failure, Resetting Pico W")
                microcontroller_timeout = 0
                try:
                    data, addr = self.sock.recvfrom_into(buffer)
                    decoded_request = buffer[:data].decode().split(':')
                    request_id = decoded_request[0]
                    request = decoded_request[1]
                    print(f"Request Received, Processing ID: {request_id}")
                    if request == 'request_data':
                        print(f"Preparing Data")
                        response = f"{request_id}:{self.moisture_value},{self.temperature}"
                        self.sock.sendto(response.encode(), addr)
                        print(f"Data Sent for: {request_id}")
                        # ~120ms to calculate new sensor values
                        self.generate_new_sensor_values()
                        print(f"Calclated new values")
                        continue
                    if request == 'reset':
            ...
            print(f"Microcontroller Error: {e}")  # Print the specific exception
            #self.reset_sensor()

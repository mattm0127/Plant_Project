...
class WaterMeLite:
    ...
    def _get_moisture_value(self, poll_num):
        """Get the average value of moisture readings (for accuracy)"""
        moisture_list = [self.ss.moisture_read() for _ in range(poll_num)]
        moisture_value = int((sum(moisture_list) / len(moisture_list)) / 10)
        self.moisture_value = moisture_value
        return moisture_value
    
    def _draw_moisture_value(self, moisture_value):
        """Draw the moisture value to the screen."""
        moisture_img = self.font.render(str(f'{moisture_value}%'), True, self.moisture_value_color)
    ...
    def _get_temperature_value_farenheit(self):
        temp_value = self.ss.get_temp()
        temp_value = int(((temp_value * self.FARENHEIT_MULTIPLIER) / self.FARENHEIT_DIVISOR) 
                         + self.FARENHEIT_OFFSET)
        return temp_value

    def _draw_soil_temp(self, temp_value):
        """Draw the soil temperature to the screen."""
        temp_img = self.font.render(str(f"{temp_value}\xb0F"), True, self.temp_value_color) # \xb0 Unicode for degree symbol
    ...
    def _update_screen(self):
        ...
        self._draw_moisture_value(self._get_moisture_value(15))
        self._draw_soil_temp(self._get_temperature_value_farenheit())
        self._draw_eye(self.left_eye_edge) # Left Eye
        self._draw_eye(self.right_eye_edge) # Right Eye
        self._draw_last_water()
        if self.phrase_timer >= self.rand_phrase_len:
            self._draw_phrase()
        pygame.display.flip()

if __name__ == '__main__':
    print('This is the updated version with modifiers.')
    wm = WaterMeLite()
    wm.run_program()